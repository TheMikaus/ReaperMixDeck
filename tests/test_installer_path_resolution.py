"""Update flow: which install.lua does the in-app Update button run?
...
def test_normalizes_separators_and_trailing_slash(lua_plain):
    ...
    assert utils.normalize_install_dir("W:\\ToolDev\\MixDeck") == "W:/ToolDev/MixDeck/"
    assert utils.normalize_install_dir("W:/ToolDev/MixDeck/") == "W:/ToolDev/MixDeck/"
...
def test_falls_back_to_current_dir_when_saved_dir_is_stale(lua_plain):
    """A saved source folder that has moved must not block the update."""
    utils = _load(lua_plain)
    current = PROJECT_DIR.as_posix() + "/"
